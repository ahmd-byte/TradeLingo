import { useState } from 'react';
import mascotImage from "../../assets/mascot.png";
import { Button } from "../ui/button";
import OnboardingFlow from "./OnboardingFlow";
import SignUpForm from "./SignUpForm";
import LoginForm from "./LoginForm";

function HeroTitle() {
  return (
    <h1 className="font-['Arimo:Bold',sans-serif] font-bold text-[72px] leading-[64px] text-center text-white tracking-[-1.8px] uppercase">
      SUPERBEAR
    </h1>
  );
}

function HeroText() {
  return (
    <div className="font-['Arimo:Bold',sans-serif] font-bold text-[20px] leading-[28px] text-[#f3ff00] text-center tracking-[0.5px] uppercase">
      <p className="mb-0">Your trades are already teaching you.</p>
      <p>We just make it obvious.</p>
    </div>
  );
}

function MascotImage() {
  return (
    <div className="w-[233px] h-[350px] rounded-[116.5px] overflow-hidden">
      <img 
        alt="SuperBear mascot" 
        className="w-full h-full object-cover" 
        src={mascotImage} 
      />
    </div>
  );
}

function JoinButton({ onClick }: { onClick: () => void }) {
  return (
    <Button 
      variant="offset" 
      className="h-[68px] rounded-[16px] w-[480px] max-w-[480px] font-['Arimo:Bold',sans-serif] text-[24px] tracking-wide uppercase"
      onClick={onClick}
    >
      SIGN UP
    </Button>
  );
}

function LoginButton({ onClick }: { onClick: () => void }) {
  return (
    <Button 
      variant="offset" 
      className="h-[68px] rounded-[16px] w-[480px] max-w-[480px] font-['Arimo:Bold',sans-serif] text-[24px] tracking-wide uppercase"
      onClick={onClick}
    >
      LOGIN
    </Button>
  );
}

function Landing({ onSignUpClick, onLoginClick }: { onSignUpClick: () => void; onLoginClick: () => void }) {
  return (
    <div className="bg-[var(--bg-primary)] flex items-center justify-center min-h-screen w-full">
      <div className="flex flex-col items-center gap-8">
        <div className="flex flex-col items-center gap-3">
          <HeroTitle />
          <HeroText />
        </div>
        
        <MascotImage />
        
        <div className="flex flex-col gap-4 items-center">
          <JoinButton onClick={onSignUpClick} />
          <LoginButton onClick={onLoginClick} />
        </div>
      </div>
    </div>
  );
}

export default function LandingPage({ onComplete }: { onComplete: () => void }) {
  const [currentScreen, setCurrentScreen] = useState<'landing' | 'signup' | 'login' | 'onboarding'>('landing');

  if (currentScreen === 'onboarding') {
    return <OnboardingFlow onComplete={onComplete} />;
  }

  if (currentScreen === 'signup') {
    return <SignUpForm onBack={() => setCurrentScreen('landing')} onSuccess={() => setCurrentScreen('onboarding')} />;
  }

  if (currentScreen === 'login') {
    return <LoginForm onBack={() => setCurrentScreen('landing')} onSuccess={onComplete} />;
  }

  return <Landing onSignUpClick={() => setCurrentScreen('signup')} onLoginClick={() => setCurrentScreen('login')} />;
}