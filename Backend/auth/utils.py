...
def hash_password(password: str) -> str:
    """
    Store password as plain text (HACKATHON / DEMO MODE).
    """
    return password
...
def verify_password(plain_password: str, stored_password: str) -> bool:
    """
    Verify password - tries bcrypt first (for old users), then plain text.
    Handles both hashed and unhashed passwords in the DB.
    """
    # 1. Try bcrypt verification (for users created before demo mode)
    try:
        if pwd_context.verify(plain_password, stored_password):
            return True
    except Exception:
        pass  # Not a bcrypt hash, fall through

    # 2. Plain text comparison (demo / hackathon mode)
